...
def predict_numbers_basic(
    ...
    **kwargs  # 添加这个来处理额外的参数
    ...
    # --- Score Regular Numbers ---
    # 1. Frequency Weight
    total_regular_draws = sum(regular_freq.values()) # Could also be len(history_data) * num_per_draw
    if total_regular_draws > 0:
        for num, freq in regular_freq.items():
            if MIN_NUMBER <= num <= MAX_NUMBER:
                ...
                regular_scores[num] += normalized_freq * WEIGHT_FREQUENCY
    ...
    for num, recency_count in recent_regular_numbers.items():
        if MIN_NUMBER <= num <= MAX_NUMBER:
            # Bonus based on how many times it appeared recently
            regular_scores[num] += (recency_count / recent_draws_count) * WEIGHT_RECENCY_HOT
    ...
    # sorted_special_candidates = sorted(special_scores.items(), key=lambda item: item[1], reverse=True)
    # predicted_special_number = sorted_special_candidates[0][0] if sorted_special_candidates else None

    # Using Counter for cleaner top-1 selection for special number
    ...
    if predicted_special_number is None and len(special_scores) > 0 : # if scores is populated but most_common failed
        predicted_special_number = list(special_scores.keys())[0] # take any
    elif predicted_special_number is None:
         predicted_special_number = random.randint(MIN_NUMBER, MAX_NUMBER) # absolute fallback
...
def predict_numbers_with_tags(
    ...
    num_to_predict: int = DEFAULT_NUM_TO_PREDICT,
    recent_draws_count: int = 10,
    tag_trend_draws: int = 20,
    weight_tag_trend: float = 0.5 # New weight factor for tag trends
) -> dict:
    """
    ...
    if total_regular_draws > 0:
        for num, freq in regular_freq.items():
            if MIN_NUMBER <= num <= MAX_NUMBER:
                regular_scores[num] += (freq / total_regular_draws) * WEIGHT_FREQUENCY
...
def predict_using_grey_model(history_data: list[dict], num_to_predict: int = 6) -> dict:
    ...
    4. 通过累减还原预测数据
    """
    def gm11(sequence: list[float]) -> float:
        """GM(1,1)灰色预测模型核心算法"""
        # 1. 累加生成
        ago = np.cumsum(sequence)
        ...
        for i in range(n-1):
            B[i,0] = -(ago[i] + ago[i+1])/2
            B[i,1] = 1
            Y[i] = sequence[i+1]
        
        # 3. 求解参数a,b
        [[a], [b]] = solve(B.T @ B, B.T @ Y)

        # Check if 'a' is close to zero to prevent division errors or instability
        if abs(a) < 1e-9:
            print(f"Warning: GM(1,1) parameter 'a' ({a}) is close to zero. GM(1,1) prediction for this window will be zero increment.")
            return 0.0 # Predicts zero increment to AGO sequence, meaning next value is same as current in AGO terms

        # 4. 预测下一个值
        next_ago = (sequence[0] - b/a) * np.exp(-a) + b/a
        return next_ago - ago[-1]  # 累减还原
    
    # 获取历史数据中的数字序列
    all_numbers = []
    for draw in history_data[-10:]:  # 使用最近10期数据
    ...
    # 对频率序列应用灰色预测
    freq_series = list(freq_dict.values())
    predictions = []
    
    # 使用滑动窗口进行预测
    window_size = 5
    for i in range(len(freq_series) - window_size):
        window = freq_series[i:i+window_size]
        try:
            pred = gm11(window)
    ...
    # 根据预测结果选择号码
    predicted_freqs = {num: freq for num, freq in zip(range(1, 50), predictions)}
    sorted_numbers = sorted(predicted_freqs.items(), key=lambda x: x[1], reverse=True)
    
    # 选择预测频率最高的号码
    regular_numbers = [num for num, _ in sorted_numbers[:num_to_predict]]
    special_number = sorted_numbers[num_to_predict][0]
